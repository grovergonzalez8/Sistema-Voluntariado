...
Project Activities, Activity Participation y Activity Attendance heredan exactamente esa autoridad del Project. Administrator usa `project.manage`; un manager contextual usa `project.read_assigned` para consultar y `project.manage_assigned` para crear, editar, completar o cancelar Activities, buscar candidatos, agregar o finalizar Participations y registrar/corregir Attendance dentro de su scope vigente. La elegibilidad inicial del Volunteer se decide mediante su Project Volunteer Assignment activa; Attendance posterior depende de la Participation existente, no de que esa Assignment continúe activa. `activity.create` y `activity.join` siguen siendo placeholders sin uso funcional: no autorizan RPC, UI ni acceso directo y no se reinterpretan en estas V1. Tasks, asistencia genérica/self-service, RSVP y self-join continúan fuera de alcance.
