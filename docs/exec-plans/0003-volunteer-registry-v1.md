...
7. Limitar V1 a `.xlsx`, 5 MiB y 1.000 filas de datos como límites técnicos, no reglas de negocio. El archivo vive solo en memoria del navegador y no se persiste. `read-excel-file` entrega el valor cacheado de una fórmula y no su expresión: no se ejecuta contenido, el preview muestra el valor resultante y la exportación vuelve a escribirlo como texto; un texto literal iniciado por `=` se rechaza. El celular debe ser una celda de texto para no fingir que pueden recuperarse `+` o ceros ya perdidos por Excel.
...
- 2026-08-16: cierre aprobado: `pnpm verify`; 139 unitarias web, 2 integración, 21 Edge Function, 13 orquestación y build; DB reset/lint y 176 pgTAP; 7 E2E incluyendo concurrencia; lockfile congelado y scans de patrones prohibidos.
- 2026-08-16: `pnpm audit --prod --audit-level high` reportó tres avisos actuales en `react-router@7.18.1` y `brace-expansion@5.0.7`. Ambas versiones ya existen idénticas en `main@c6f7392`; las dependencias Excel no aparecen en las rutas. No se actualizaron por la prohibición de mezclar deuda no relacionada.
...
La V1 quedó implementada en `feat/volunteer-registry-v1` sin tocar las migraciones históricas, sin merge ni push. Commit técnico: `01c127d`; el cierre documental se registra en un commit posterior.
