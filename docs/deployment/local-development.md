...
`verify` es el gate estático y no requiere Docker. `test:functions` prueba el handler puro; DB, concurrencia y E2E requieren Supabase local. `projects:test:concurrency` se ejecuta después de `db:reset`, controla conexiones PostgreSQL independientes mediante `pg_blocking_pids`, valida ambos órdenes assign/close para participaciones y scopes, el duplicado de scope y revocación/mutación contextual, y elimina sus fixtures, sesiones y locks. `account-lifecycle:test` ejecuta funciones, reconstruye la base local, aplica lint SQL, corre pgTAP, prueba la concurrencia de Projects y finaliza con E2E; no depende del estado residual de una ejecución anterior.
