...
| Carrera Participation/eligibilidad           | orden account–scope–Project–Assignment–Activity–Participation, rechecks e índice parcial                                     | RSVP futuro                          |
| Carrera Attendance/lifecycle                 | orden account–scope–Project–Activity–Participation–Attendance, precondición optimista y harness con conexiones reales        | bulk/self-service futuros            |
...
Projects mantiene `project.manage` como autoridad global exclusiva de administrator. `project_manager` recibe `project.read_assigned` y `project.manage_assigned`, pero cada RPC contextual exige además cuenta activa, rol vigente y scope activo para el proyecto. `projects`, `project_volunteer_assignments`, `project_manager_assignments`, `project_activities`, `project_activity_participations` y `project_activity_attendances` mantienen RLS sin policies ni grants de tabla; las RPC proyectan solo campos aprobados. Activity nace `scheduled`, terminaliza una vez y no admite DELETE. Activity Participation enlaza exclusivamente Activity y `volunteers`, exige Assignment activa del Project exacto al crear, es monotónica y no admite DELETE/reactivación. Attendance enlaza exclusivamente una Participation, persiste solo `present|absent`, exige Project activo y Activity completed, no requiere Assignment vigente y no admite DELETE. La mutación bloquea account–scope–Project–Activity–Participation–Attendance y compara `expected_status` bajo lock; close, scope removal y complete Activity se revalidan después de esperar. Auditoría y read models usan IDs/estados técnicos y el nombre mínimo ya autorizado por Participation; no copian email, teléfono, `phone_match_key`, contenido Activity ni metadata Auth.
